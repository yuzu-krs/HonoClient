...
public class Axolotl extends Animal implements VariantHolder<Axolotl.Variant>, Bucketable {
    ...
    public void saveToBucketTag(ItemStack p_149187_) {
        ...
        CustomData.update(DataComponents.BUCKET_ENTITY_DATA, p_149187_, p_359202_ -> {
            p_359202_.putInt("Variant", this.getVariant().getId());
            p_359202_.putInt("Age", this.getAge());
            Brain<?> brain = this.getBrain();
            if (brain.hasMemoryValue(MemoryModuleType.HAS_HUNTING_COOLDOWN)) {
                p_359202_.putLong("HuntingCooldown", brain.getTimeUntilExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN));
            }
        });
    }
