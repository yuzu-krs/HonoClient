...
public record RecipeDisplayEntry(
    ...
    public boolean canCraft(StackedItemContents p_367444_) {
        if (this.craftingRequirements.isEmpty()) {
            ...
            List<StackedContents.IngredientInfo<Holder<Item>>> list = this.craftingRequirements.get().stream().map(PlacementInfo::ingredientToContents).toList();
            return p_367444_.canCraft(list, null);
        }
    }
}