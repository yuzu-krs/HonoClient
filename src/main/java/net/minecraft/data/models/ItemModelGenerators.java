...
public class ItemModelGenerators {
    ...
    private JsonObject generateBaseArmorTrimTemplate(ResourceLocation p_266939_, Map<TextureSlot, ResourceLocation> p_267324_, ResourceLocation p_364662_) {
        ...
        for (ItemModelGenerators.TrimModelData itemmodelgenerators$trimmodeldata : GENERATED_TRIM_MODELS) {
            ...
            jsonobject1.addProperty("model", this.getItemModelForTrimMaterial(p_266939_, itemmodelgenerators$trimmodeldata.name(p_364662_)).toString());
    ...
    private void generateArmorTrims(Item p_363904_, ResourceLocation p_365082_, EquipmentModel p_368656_, EquipmentSlot p_368560_) {
        List<EquipmentModel.Layer> list = p_368656_.getLayers(EquipmentModel.LayerType.HUMANOID);
        if (!list.isEmpty()) {
            boolean flag = list.size() == 2 && list.getFirst().dyeable().isPresent();
            ResourceLocation resourcelocation = ModelLocationUtils.getModelLocation(p_363904_);
            ResourceLocation resourcelocation1 = TextureMapping.getItemTexture(p_363904_);
            ResourceLocation resourcelocation2 = TextureMapping.getItemTexture(p_363904_, "_overlay");
            if (flag) {
                ModelTemplates.TWO_LAYERED_ITEM
                    .create(
                        ...
                        this.output,
                        (p_358337_, p_358338_) -> this.generateBaseArmorTrimTemplate(p_358337_, p_358338_, p_365082_)
                    );
            } else {
                ModelTemplates.FLAT_ITEM
                    .create(
                        ...
                        this.output,
                        (p_358334_, p_358335_) -> this.generateBaseArmorTrimTemplate(p_358334_, p_358335_, p_365082_)
                    );
            }
            String s = switch (p_368560_) {
                case HEAD -> "helmet";
            ...
            for (ItemModelGenerators.TrimModelData itemmodelgenerators$trimmodeldata : GENERATED_TRIM_MODELS) {
                String s1 = itemmodelgenerators$trimmodeldata.name(p_365082_);
    ...
    static record TrimModelData(String name, float itemModelIndex, Map<ResourceLocation, String> overrideArmorMaterials) {
        public String name(ResourceLocation p_363786_) {
            return this.overrideArmorMaterials.getOrDefault(p_363786_, this.name);
        }
    }
}